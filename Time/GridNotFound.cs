    ...
    public partial class GridNotFound : Form
    {
        ...
        private void GridNotFound_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < Check.listNotFound.Count; i++)
            {
                ...
                dataGridView1.Rows[row].Cells[0].Value = Check.listNotFound[i].date.ToShortDateString();
                dataGridView1.Rows[row].Cells[1].Value = Check.listNotFound[i].hour;
                dataGridView1.Rows[row].Cells[2].Value = Check.listNotFound[i].sinif;
            }
